...
function execute(opts, env, config) {
  ...
  // This is what actually loads up the wiafile
  var exported = require(env.configPath);

  registerExports(gulpInst, exported);

  // Always unmute stdout after wiafile is required
  stdout.unmute();
  ...
  process.nextTick(function() {
    ...
    try {
      log.info('Using wiafile', ansi.magenta(tildify(env.configPath)));
      ...
      log.error('To list available tasks, try running: wia --tasks');
      exit(1);
    }
  });
}
