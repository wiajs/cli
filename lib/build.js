...
var makeTitle = require('./shared/make-title');
var cliOptions = require('./shared/cli-options');
var completion = require('./shared/completion');
var verifyDeps = require('./shared/verify-dependencies');
var cliVersion = require('../package.json').version;
var getBlacklist = require('./shared/get-blacklist');
var toConsole = require('./shared/log/to-console');

var loadConfigFiles = require('./shared/config/load-files');
var mergeConfigToCliFlags = require('./shared/config/cli-flags');
var mergeConfigToEnvFlags = require('./shared/config/env-flags');

// Logging functions
var logVerify = require('./shared/log/verify');
var logBlacklistError = require('./shared/log/blacklist-error');

// Get supported ranges
var ranges = fs.readdirSync(__dirname + '/versioned/');
...
function run(file) {
  cli.prepare({
    cwd: opts.cwd,
    configPath: file || opts.wiafile,
    require: opts.require,
    completion: opts.completion,
  }, function(env) {
    var cfgLoadOrder = ['home', 'cwd'];
    var cfg = loadConfigFiles(env.configFiles['.wia'], cfgLoadOrder);
...
function handleArguments(env) {
  ...
  // We use the process.env so the user's wiafile
  ...
  if (!env.configPath) {
    log.error(ansi.red('No wiafile found'));
    exit(1);
  }

  // Chdir before requiring wiafile to make sure
  ...
  var versionedDir = path.join(__dirname, '/versioned/', range, '/');
